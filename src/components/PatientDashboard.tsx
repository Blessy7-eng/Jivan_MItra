...
export default function PatientDashboard({ refreshTrigger }: { refreshTrigger?: number }) {
  ...
  const getTimingIcon = (timing: string) => {
    switch(timing.toLowerCase()) {
       ...
       case "evening": return <Coffee size={14} className="text-slate-600" />;
       case "night": return <Moon size={14} className="text-[#0D47A1]" />;
  ...
  return (
    <section className="space-y-4">
      {prescriptions.length === 0 ? (
        <div className="bg-slate-50 border border-dashed border-slate-200 p-8 rounded-2xl text-center text-slate-400 font-medium">
            ...
            <div key={rx.id} className="bg-white rounded-xl p-4 border border-slate-200 hover:shadow-md transition-shadow group">
              <div className="flex items-start justify-between">
                 <div className="flex gap-4 items-start">
                    <div className="bg-slate-50 border border-slate-100 p-2.5 rounded-xl text-[#0D47A1] group-hover:bg-[#0D47A1] group-hover:text-white transition-colors">
                     <Pill size={20} />
                   </div>
                   <div>
                     <h3 className="text-base font-bold text-[#0D47A1] leading-tight mb-1">{rx.extractedData?.drugName || "Unknown Medicine"}</h3>
                 ...
                 <button 
                   ...
                   className="text-slate-400 hover:text-[#0D47A1] hover:bg-blue-50 p-2 rounded-xl transition-colors"
                 >
                   <Camera size={18} />
                 </button>
              </div>
              ...
              {verifyingPillFor === rx.id && (
                 <div className="mt-4 ml-14 bg-slate-50 p-3 rounded-xl border border-slate-200">
                    <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3">AI Pill Verification</h4>
                    <input 
                      type="file" 
                      accept="image/*" 
                      className="hidden" 
                    ...
                    {!verificationResult && !isVerifying && (
                      <button 
                         ...
                         className="w-full bg-white border border-slate-200 hover:border-[#0D47A1] hover:text-[#0D47A1] text-slate-600 font-bold text-xs py-2.5 rounded-xl flex justify-center items-center gap-2 transition-colors shadow-sm uppercase tracking-widest"
                      >
                    ...
                    {isVerifying && (
                       <div className="flex justify-center items-center py-3 text-xs font-bold text-[#0D47A1] uppercase tracking-widest gap-2">
                    ...
                    {verificationResult && (
                       <div className={`mt-2 text-sm font-semibold p-3 rounded-lg border flex gap-2 items-start bg-white ${verificationResult.toUpperCase().includes('YES') ? 'border-green-200' : 'border-red-200'}`}>
