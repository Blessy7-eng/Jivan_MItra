    ...
    >
      <div className="bg-white rounded-3xl w-full max-w-md p-8 relative overflow-hidden shadow-2xl animate-in fade-in zoom-in-95">
          ...
          {!isLogin && (
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-2xl py-3.5 pl-11 pr-4 font-semibold text-slate-800 focus:ring-2 focus:ring-[#0D47A1] focus:border-transparent outline-none transition-all"
                value={name}
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button type="button" onClick={() => loginWithGoogle()} className="bg-white border border-slate-200 text-slate-700 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-slate-50 transition-colors shadow-sm">
        ...
        <p className="mt-8 text-center text-sm font-semibold text-slate-500">
          ...
          <button onClick={() => setIsLogin(!isLogin)} className="text-[#0D47A1] hover:underline font-bold">
